#' assess the relation between temperature and trait and extract the parameters
#'
...
#'
rel_clim_trait <- function(biol_data_yT,
                         ID,
  ...
  ## think this is not needed, was needed for the climin, will drop for now
  ...
  ## !!! check: I think this forst condition with datafrmae as the data can be dropped
  if(class(clim_data) == 'data.frame'){
    ...
    ## extract data from the weather data for all the necessary dates for the site according to biol_data set
    ...
    Clim <- data.frame(Date = seq(as.Date(paste('01', '01', min(biol_data$Year), sep = '/'), format = '%d/%m/%Y'),
                                  as.Date(paste('01', '12', max(biol_data$Year), sep = '/'), format = '%d/%m/%Y'), 'day'),
                       Temp = NA)
